...
class Augmenter(DTROS):
    ...
    def __init__(self, node_name):
        ...
        super(Augmenter, self).__init__(node_name=node_name, node_type=NodeType.GENERIC)
        ...
        try:
            mapfile_dir = ("/code/catkin_ws/src/cra1_basic/packages/augmented_reality_basics/map/{}.yaml".format(self.mapfile))
            self.map = self.yaml_load_file(mapfile_dir)
        ...
        self.pub = rospy.Publisher("/{}/augmented_reality_basics_node/{}/image/compressed".format(self.veh_name, self.mapfile), CompressedImage, queue_size=1)
    ...
    def load_homography(self):
        ...
        filename = ("/code/catkin_ws/src/cra1_basic/calibrations/camera_extrinsic/"+ self.veh_name + ".yaml")
        # filename = ("/data/config/calibrations/camera_extrinsic/"+ self.veh_name + ".yaml")

        rospy.loginfo("Using extrinsic calibration of " + self.veh_name)
        data = self.yaml_load_file(filename)
        rospy.loginfo("Loaded homography for {}".format(self.veh_name))
        return np.array(data['homography']).reshape((3,3))

    def load_camera_info(self):
        '''Load camera intrinsics'''
        filename = ("/code/catkin_ws/src/cra1_basic/calibrations/camera_intrinsic/" + self.veh_name + ".yaml")
        # filename = ("/data/config/calibrations/camera_intrinsic/" + self.veh_name + ".yaml")
        calib_data = self.yaml_load_file(filename)
        cam_info = CameraInfo()
    ...
    def callback(self, data):
        ...
        if self.iter  == 0:
            cv2.imwrite("/code/catkin_ws/src/cra1_basic/calibrations/testimgs_ns.png", img_undistorted)
